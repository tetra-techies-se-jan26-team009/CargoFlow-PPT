...
export function AddAgentModal({ onClose }) {
    ...
    const validate = () => {
        const e = {};
        if (!f.name) e.name = "Agent Name is required";
        if (!f.email) {
    ...
    const submit = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                city: f.city,
                password: f.password // ✅ Fixed: Was hardcoded to "123" previously
            };
            ...
            if (Array.isArray(detail)) {
                ...
                setApiErr(err?.message || "Failed to add agent");
            }
        } finally {
            setLoading(false);
        }
        ...
        <Modal onClose={onClose} width={420}>
            ...
            <div style={{ padding: "32px 28px", fontFamily: "'DM Sans', sans-serif", background: "#FFFFFF", borderRadius: 16 }}>
                <div style={{ textAlign: "center", marginBottom: 24 }}>
                    ...
                    <div style={{ fontSize: 20, fontWeight: 800, color: "#111827", marginBottom: 6, letterSpacing: "-0.3px" }}>Agent Added!</div>
                    <div style={{ fontSize: 13, color: "#6B7280" }}>Delivery agent successfully registered</div>
                </div>

                <div style={{ background: "#F9FAFB", border: "1px solid #E5E7EB", borderRadius: 12, overflow: "hidden", marginBottom: 24 }}>
                    <div style={{ padding: "16px 20px", display: "grid", gridTemplateColumns: "1fr", gap: "12px" }}>
                        <div>
                            <div style={{ fontSize: 10, color: "#6B7280", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 4 }}>Agent Name</div>
                            <div style={{ fontSize: 14, fontWeight: 600, color: "#111827" }}>{f.name}</div>
                        </div>
                        <div style={{ height: 1, background: "#E5E7EB" }} />
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                            <div>
                                <div style={{ fontSize: 10, color: "#6B7280", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 4 }}>Email</div>
                                <div style={{ fontSize: 13, fontWeight: 500, color: "#374151" }}>{f.email}</div>
                            </div>
                            <div>
                                <div style={{ fontSize: 10, color: "#6B7280", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 4 }}>Base City</div>
                                <div style={{ fontSize: 13, fontWeight: 500, color: "#374151" }}>{f.city}</div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <Modal onClose={onClose} width={500}>
            ...
            <div style={{ background: "#FFFFFF", borderRadius: 16, overflow: "hidden", display: "flex", flexDirection: "column" }}>
                ...
                <div style={{ padding: "24px 28px 20px", background: "#F9FAFB", borderBottom: "1px solid #E5E7EB", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                        <div style={{ width: 40, height: 40, borderRadius: 10, background: "#2563EB", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 6px -1px rgba(37, 99, 235, 0.2)" }}>
                            <Ico d={IC.agent} s={20} c="#FFFFFF" />
                        </div>
                        <div>
                            <div style={{ fontSize: 18, fontWeight: 800, color: "#111827", letterSpacing: "-0.3px", marginBottom: 2 }}>Add New Agent</div>
                            <div style={{ fontSize: 11, color: "#6B7280", fontFamily: "'DM Mono', monospace", letterSpacing: "0.5px" }}>FIELD OPERATIONS</div>
                        </div>
                    </div>
                ...
                <div style={{ padding: "24px 28px", display: "flex", flexDirection: "column", gap: 20 }}>
                    ...
                    <F label="Agent Full Name" icon={IC.user} required error={errs.name}>
                        <input className="asf-in" value={f.name} onChange={e => set("name", e.target.value)}
                            placeholder="e.g. Ravi Kumar" style={iBase(errs.name)} onFocus={oF} onBlur={oB} />
                    </F>

                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                        <F label="Email Address" icon={IC.mail} required error={errs.email}>
                            <input className="asf-in" type="email" value={f.email} onChange={e => set("email", e.target.value)}
                                placeholder="agent@cargoflow.in" style={iBase(errs.email)} onFocus={oF} onBlur={oB} />
                        </F>
                        <F label="Phone Number" icon={IC.phone} required error={errs.phone}>
                            <input className="asf-in" type="number" value={f.phone} onChange={e => set("phone", e.target.value)}
                                placeholder="98765 43210" maxLength={10} style={iBase(errs.phone)} onFocus={oF} onBlur={oB} />
                        </F>
                    </div>

                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                        <F label="Base City" icon={IC.pin} required error={errs.city}>
                            <input className="asf-in" value={f.city} onChange={e => set("city", e.target.value)}
                                placeholder="e.g. Chennai" style={iBase(errs.city)} onFocus={oF} onBlur={oB} />
                        </F>
                        <F label="Temporary Password" icon={IC.lock} required error={errs.password} hint="Agent will reset on first login">
                            <input className="asf-in" type="password" value={f.password} onChange={e => set("password", e.target.value)}
                                placeholder="••••••••" style={iBase(errs.password)} onFocus={oF} onBlur={oB} />
                        </F>
                    </div>
                ...
                <div style={{ padding: "20px 28px", borderTop: "1px solid #E5E7EB", display: "flex", alignItems: "center", justifyContent: "space-between", background: "#F9FAFB" }}>
                    <div style={{ fontSize: 13 }}>
                        {Object.keys(errs).length > 0 && (
                            <span style={{ color: "#DC2626", display: "flex", alignItems: "center", gap: 6, fontWeight: 500 }}>
                                <Ico d={IC.alert} s={14} c="#DC2626" /> Fix {Object.keys(errs).length} error{Object.keys(errs).length > 1 ? "s" : ""}
                            </span>
                        )}
                    </div>
                    <div style={{ display: "flex", gap: 12 }}>
                        ...
                        <button onClick={submit} disabled={loading}
                            style={{ padding: "10px 24px", borderRadius: 8, background: loading ? "#93C5FD" : "#2563EB", border: "1px solid transparent", color: "#FFFFFF", fontSize: 14, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", gap: 8, minWidth: 140, justifyContent: "center", boxShadow: loading ? "none" : "0 4px 6px -1px rgba(37, 99, 235, 0.2)", transition: "all 0.2s" }}>
                            {loading ? (
                                <>
                                ...
                                <><Ico d={IC.check} s={15} c="white" /> Add Agent</>
                            )}
                        </button>
                    </div>
                </div>
