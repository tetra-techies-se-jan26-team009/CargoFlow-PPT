...
export default function ClientsPage() {
      ...
      setClientList(
        (res?.clients || []).map((c) => {
          ...
          return {
            ...
            business_name: c.business || "N/A",
            joined_date: c.joined
  ...
  const filtered = clientList.filter((c) => {
    ...
    const matchSearch =
      !search ||
      [c.business_name, c.contact_person, c.city].some((v) =>
    ...
    <div className="flex flex-col h-screen bg-[#F1F5F9] font-sans overflow-hidden">
      ...
      <main className="flex-1 overflow-hidden px-[100px] py-6 flex flex-col gap-5">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-[22px] font-extrabold text-[#0F172A] m-0 tracking-tight">
              Manage Clients
            </h1>
            <p className="text-xs text-[#94A3B8] mt-1">
              {data.total_clients} registered business clients
            </p>
          </div>
          <button
            onClick={() => setModal("addClient")}
            className="flex items-center gap-1.5 px-[18px] py-[9px] bg-[#2563EB] text-white text-xs font-semibold rounded-lg hover:bg-blue-700"
          >
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            ...
            {
              label: "Total Revenue",
              val: `₹${data.total_revenue.toLocaleString()}`,
              color: "text-[#7C3AED]",
            },
          ].map((k) => (
            <div
              key={k.label}
              className="bg-white rounded-xl p-[14px_18px] border border-[#F1F5F9] shadow-sm"
            >
              <div
                className={`text-[26px] font-extrabold ${k.color} tracking-tighter`}
              >
                {k.val}
              </div>
              <div className="text-[11px] text-[#94A3B8] mt-1 uppercase font-bold">
                {k.label}
              </div>
            </div>
          ))}
        ...
        <div className="flex gap-2.5 items-center">
          <input
            className="flex-1 max-w-[300px] border border-[#E2E8F0] rounded-lg px-3 py-2 text-xs focus:outline-none bg-white font-medium"
          ...
          {["All", "Active", "Overdue", "Inactive"].map((f) => (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-lg border text-xs font-semibold ${filter === f ? "bg-[#EFF6FF] border-[#2563EB] text-[#2563EB]" : "bg-white border-[#E2E8F0] text-[#64748B]"}`}
            >
              {f}
            </button>
          ))}
            ...
            <table className="w-full border-collapse text-left">
              <thead className="sticky top-0 bg-[#F8FAFC] z-10 shadow-sm">
                <tr>
                  {[
                    ...
                    <th
                      key={h}
                      className="px-4 py-2.5 text-[11px] font-semibold text-[#94A3B8] uppercase border-b border-[#F1F5F9] whitespace-nowrap"
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {paginatedData.map((c) => (
                  <tr
                    key={c.id}
                    className="border-b border-[#F8FAFC] hover:bg-[#F8FAFC] transition-colors group"
                  >
                    <td className="p-4">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8.5 h-8.5 rounded-lg bg-[#EFF6FF] text-[#2563EB] flex items-center justify-center text-xs font-bold shrink-0">
                          {c.business_name[0]}
                        </div>
                        <div>
                    ...
                    <td className="p-4 text-xs text-[#334155]">
                      {c.contact_person}
                    </td>
                    <td className="p-4 text-[11px] text-[#64748B]">
                      {c.email}
                    </td>
                    <td className="p-4 text-[11px] text-[#64748B] whitespace-nowrap">
                      {c.phone}
                    </td>
                    ...
                    <td className="p-4">
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${statusMeta[c.status]?.bg} ${statusMeta[c.status]?.text}`}
                      >
                        {c.status}
                      </span>
                    </td>
                    ...
                    <td className="p-4">
                      <button
                        ...
                        className="px-4 py-1.5 border border-[#E2E8F0] rounded-md bg-white text-[#334155] text-[11px] font-bold hover:bg-gray-50 uppercase tracking-tighter"
                      >
                        View
                      </button>
                    </td>
          ...
          <div className="p-4 border-t border-[#F1F5F9] flex justify-between items-center bg-white">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-3 py-1 text-xs border border-[#E2E8F0] rounded-md hover:bg-gray-50 disabled:opacity-30 font-semibold"
              >
                Previous
              </button>
              <button
          ...
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[85vh] overflow-hidden border border-slate-200">
              ...
              <div className="flex items-center gap-3">
                {!isEditing ? (
                  ...
                  <button
                    ...
                    disabled={isUpdating}
                    className="px-4 py-2 bg-blue-600 text-white text-[10px] font-black rounded-lg hover:bg-blue-700 uppercase tracking-widest disabled:opacity-50 transition-all"
                  >
                ...
                <button
                  ...
                  className="text-slate-400 hover:text-black text-2xl ml-2 leading-none"
                >
                  &times;
                </button>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
              ...
              <div className="flex items-center gap-6 mb-10 pb-8 border-b border-slate-50">
                ...
                <div
                  className={`w-20 h-20 rounded-2xl flex items-center justify-center text-3xl font-black shrink-0 transition-colors ${isClientActive ? "bg-blue-50 text-blue-600" : "bg-red-50 text-red-600"}`}
                >
                ...
                <div>
                  ...
                  <div className="mt-2 flex gap-2">
                    ...
                    <span
                      className={`text-[10px] font-black px-2 py-1 rounded-md uppercase tracking-wider ${isClientActive ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}
                    >
              ...
              <div className="mt-12 p-6 bg-red-50/50 rounded-2xl border border-red-100">
                <div className="flex justify-between items-center">
                  <div>
                    <h4 className="text-xs font-black text-red-700 uppercase tracking-widest">
                      Danger Zone
                    </h4>
                    <p className="text-[10px] text-red-600 mt-1 font-medium">
                      Temporary disable or remove this client's access.
                    </p>
                  </div>
                  <button
                    onClick={handleToggleStatus}
                    disabled={isUpdating}
                    className={`px-5 py-2 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all border shadow-sm ${
                      isClientActive
